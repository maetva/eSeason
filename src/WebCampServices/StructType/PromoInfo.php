...
class PromoInfo extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 1
     * @var \Maetva\ESeason\WebCampServices\StructType\Promo[]
     */
     ...
     * @param \Maetva\ESeason\WebCampServices\StructType\Promo[] $promotion
     */
     ...
     * @param float $totalAmount
     * @return \Maetva\ESeason\WebCampServices\StructType\PromoInfo
     */
    ...
    public static function validatePromotionForArrayConstraintsFromSetPromotion(?array $values = []): string
    {
        ...
        foreach ($values as $promoInfoPromotionItem) {
            ...
            if (!$promoInfoPromotionItem instanceof \Maetva\ESeason\WebCampServices\StructType\Promo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The promotion property can only contain items of type \Maetva\ESeason\WebCampServices\StructType\Promo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Maetva\ESeason\WebCampServices\StructType\Promo $item
     * @return \Maetva\ESeason\WebCampServices\StructType\PromoInfo
     */
    public function addToPromotion(\Maetva\ESeason\WebCampServices\StructType\Promo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Maetva\ESeason\WebCampServices\StructType\Promo) {
            throw new InvalidArgumentException(sprintf('The promotion property can only contain items of type \Maetva\ESeason\WebCampServices\StructType\Promo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->promotion[] = $item;
